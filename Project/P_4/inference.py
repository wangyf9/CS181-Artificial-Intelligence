...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: BayesNet, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        "*** YOUR CODE HERE ***"
        #Get Factor
        factor = bayesNet.getAllCPTsWithEvidence(evidenceDict)          #All the factor
        # join and eliminate all factors by variable
        for var in eliminationOrder:                                    
            factor, joinedFactor = joinFactorsByVariable(factor, var)   #joinFactorsByVariable to join all of the factors for eliminate variable
            ##print("123",joinedFactor)
            ##print("456",factor)
            if len(joinedFactor.unconditionedVariables()) != 1:         ##not only one unconditioned var 
                new_factor = eliminate(joinedFactor, var)
                factor.append(new_factor)
        return normalize(joinFactors(factor))                           #construct a new factor and normalize
        "*** END YOUR CODE HERE ***"
