...
def constructBayesNet(gameState: GameState):
    ...
    "*** YOUR CODE HERE ***"
    for housePos in gameState.getPossibleHouses(): ##Find all observations
        for obsPos in gameState.getHouseWalls(housePos):
            obsVar = OBS_VAR_TEMPLATE % obsPos
            obsVars.append(obsVar)
            variableDomainsDict[obsVar] = OBS_VALS   ##Add the possible values for variables
    ...
    edges.append((X_POS_VAR,GHOST_HOUSE_VAR))       ###Add all the edge, that is the relation between variables
    edges.append((X_POS_VAR,FOOD_HOUSE_VAR))  
    edges.append((Y_POS_VAR,GHOST_HOUSE_VAR))
    edges.append((Y_POS_VAR,FOOD_HOUSE_VAR))  
    for ob in obsVars:
        edges.append((FOOD_HOUSE_VAR,ob))
        edges.append((GHOST_HOUSE_VAR,ob))      
    "*** END YOUR CODE HERE ***"
