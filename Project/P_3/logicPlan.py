...
def sentence1() -> Expr:
    ...
    C = Expr('C')
    listres = []
    listres.append(disjoin(A, B))
    listres.append((~A) % disjoin((~B), C))
    listres.append(disjoin((~A), (~B), C))
    return conjoin(listres)
...
def sentence2() -> Expr:
    ...
    D = Expr('D')
    listres = []
    listres.append(C % disjoin(B, D))
    listres.append(A >> conjoin((~B), (~D)))
    listres.append((~conjoin(B, (~C))) >> A)
    listres.append((~D) >> C)
    return conjoin(listres)
    "*** END YOUR CODE HERE ***"
...
def sentence3() -> Expr:
    ...
    listres = []
    listres.append(PacmanAlive_1 % (disjoin((conjoin(PacmanAlive_0, (~PacmanKilled_0))), conjoin((~PacmanAlive_0), PacmanBorn_0))))
    listres.append(~conjoin(PacmanAlive_0, PacmanBorn_0))
    listres.append(PacmanBorn_0)
    return conjoin(listres)
    "*** END YOUR CODE HERE ***"
...
def entails(premise: Expr, conclusion: Expr) -> bool:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    return not findModel(conjoin(premise,~conclusion)) ##directly A -> B is false and ~A | B is also false. The final choice is ~(A & ~B)
    "*** END YOUR CODE HERE ***"
...
def atLeastOne(literals: List[Expr]) -> Expr:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    return disjoin(literals) ##one true, the list is true
    "*** END YOUR CODE HERE ***"
...
def atMostOne(literals: List[Expr]) -> Expr:
    ...
    """
    "*** BEGIN YOUR CODE HERE ***"
    pair = itertools.combinations(literals, 2)
    clauses = [(~pair_one[0]| ~pair_one[1]) for pair_one in pair]
...
def pacmanSuccessorAxiomSingle(x: int, y: int, time: int, walls_grid: List[List[bool]]=None) -> Expr:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    current_act = PropSymbolExpr(pacman_str, x , y, time = now)
    successor = current_act % disjoin(possible_causes)
    return successor
    "*** END YOUR CODE HERE ***"
...
def pacphysicsAxioms(t: int, all_coords: List[Tuple], non_outer_wall_coords: List[Tuple], walls_grid: List[List] = None, sensorModel: Callable = None, successorAxioms: Callable = None) -> Expr:
    ...
    for i in range(len(all_coords)):
        pos = all_coords[i]
        #first sentence
        pacphysics_sentences.append(PropSymbolExpr(wall_str, pos[0], pos[1]) >> ~PropSymbolExpr(pacman_str, pos[0], pos[1], time = t))
        if pos in non_outer_wall_coords:
            game_pos.append(PropSymbolExpr(pacman_str, pos[0], pos[1], time = t))
    #second
    pacphysics_sentences.append(exactlyOne(game_pos))

    nextstep = []
    for i in range(len(DIRECTIONS)):
        nextstep.append(PropSymbolExpr(DIRECTIONS[i], time = t))
    #third
    pacphysics_sentences.append(exactlyOne(nextstep))
...
def checkLocationSatisfiability(x1_y1: Tuple[int, int], x0_y0: Tuple[int, int], action0, action1, problem):
    ...
    "*** BEGIN YOUR CODE HERE ***"
    #construct the KB
    KB.append(conjoin(pacphysicsAxioms(0, all_coords, non_outer_wall_coords, walls_grid = walls_grid, successorAxioms = None)))
    KB.append(conjoin(pacphysicsAxioms(1, all_coords, non_outer_wall_coords, walls_grid = walls_grid, successorAxioms = allLegalSuccessorAxioms)))
    KB.append(PropSymbolExpr(pacman_str, x0, y0, time = 0))
    KB.append(PropSymbolExpr(action0, time = 0))
    KB.append(PropSymbolExpr(action1, time = 1))
    #use KB to construct model
...
def positionLogicPlan(problem) -> List:
    ...
    for t in range(50):
        ...
        for x, y in non_wall_coords:
            game_pos.append(PropSymbolExpr(pacman_str, x, y, time = t)) 
        KB.append(exactlyOne(game_pos))
        ...
        if t > 0:
            for x, y in non_wall_coords:
                successoraxioms = pacmanSuccessorAxiomSingle(x, y, t, walls_grid)
...
def foodLogicPlan(problem) -> List:
    ...
    for x, y in food:
        food_pos.append(PropSymbolExpr(food_str, x, y , time = 0))
    ...
    for t in range(50):
        ...
        if t > 0:
            for x, y in non_wall_coords:
                successoraxioms = pacmanSuccessorAxiomSingle(x, y, t, walls)
                KB.append(conjoin(successoraxioms))
        #food successor: the relation among Food[x,y]_t+1 and Food[x,y]_t and Pacman[x,y]_t
        food_successor_axioms = []
        food_all_eaten = []
        for x, y in food:
            food_all_eaten.append(~PropSymbolExpr(food_str, x, y , time = t))
            sen1 = ~PropSymbolExpr(pacman_str, x, y, time = t)
            sen2 = PropSymbolExpr(food_str, x, y, time = t)
            sen3 = PropSymbolExpr(food_str, x, y, time = t + 1)
            final = conjoin(sen1, sen2) >> sen3
            food_successor_axioms.append(final)
        KB.append(conjoin(food_successor_axioms))
        #model

        model = findModel(conjoin(conjoin(food_all_eaten), conjoin(KB)))
...
def localization(problem, agent) -> Generator:
    ...
    for x, y in walls_list:
        wall_pos.append(PropSymbolExpr(wall_str, x, y))
    KB.append(conjoin(wall_pos))
    for x, y in all_coords:
        if((x, y) not in walls_list):
            not_wall_pos.append(~PropSymbolExpr(wall_str, x, y))
    KB.append(conjoin(not_wall_pos))

    for t in range(agent.num_timesteps):
        #pacphysics
        if t > 0:
            KB.append(conjoin(pacphysicsAxioms(t, all_coords, non_outer_wall_coords, walls_grid = walls_grid, sensorModel = sensorAxioms, successorAxioms = allLegalSuccessorAxioms)))
        else:
            KB.append(conjoin(pacphysicsAxioms(t, all_coords, non_outer_wall_coords, walls_grid = walls_grid, sensorModel = sensorAxioms, successorAxioms = None)))
        ##action
        KB.append(PropSymbolExpr(agent.actions[t], time = t))
        #percepts
        percept_information = fourBitPerceptRules(t, agent.getPercepts())
        KB.append(percept_information)

        #possible pacman locations with updated KB.
        possible_locations = []
        for x, y in non_outer_wall_coords:
            sen1 = PropSymbolExpr(pacman_str, x, y, time = t)
            sen2 = ~PropSymbolExpr(pacman_str, x, y, time = t)
            if(entails(conjoin(KB), sen1)):
                KB.append(sen1)
            if(entails(conjoin(KB), sen2)):
                KB.append(sen2)

            #If there exists a satisfying assignment where Pacman is at (x, y) at time t
            model = findModel(PropSymbolExpr(pacman_str, x, y, time = t) & conjoin(KB))
            if(model != False):
...
def mapping(problem, agent) -> Generator:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    #Initialize
    pacman_pos = PropSymbolExpr(pacman_str, pac_x_0, pac_y_0, time = 0)
    ...
    for t in range(agent.num_timesteps):
        #pacphysics
        if t > 0:
            KB.append(conjoin(pacphysicsAxioms(t, all_coords, non_outer_wall_coords, walls_grid = known_map, sensorModel = sensorAxioms, successorAxioms = allLegalSuccessorAxioms)))
        else:
            KB.append(conjoin(pacphysicsAxioms(t, all_coords, non_outer_wall_coords, walls_grid = known_map, sensorModel = sensorAxioms, successorAxioms = None)))
        ##action
        KB.append(PropSymbolExpr(agent.actions[t], time = t))
        #percepts
        percept_information = fourBitPerceptRules(t, agent.getPercepts())
        ...
        for x, y in non_outer_wall_coords:
            sen1 = PropSymbolExpr(wall_str, x, y)
            sen2 = ~PropSymbolExpr(wall_str, x, y)
            if(entails(conjoin(KB), sen1)):
                KB.append(sen1)
                known_map[x][y] = 1
            if(entails(conjoin(KB), sen2)):
...
def slam(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        ...
        KB.append(PropSymbolExpr(agent.actions[t], time = t))
        #percepts
        percept_information = numAdjWallsPerceptRules(t, agent.getPercepts())
        KB.append(percept_information)
        ...
        for x, y in non_outer_wall_coords:
            sen1 = PropSymbolExpr(wall_str, x, y)
            sen2 = ~PropSymbolExpr(wall_str, x, y)
            sen3 = PropSymbolExpr(pacman_str, x, y, time = t)
            sen4 = ~PropSymbolExpr(pacman_str, x, y, time = t)
            if(entails(conjoin(KB), sen1)):
                KB.append(sen1)
                known_map[x][y] = 1
            if(entails(conjoin(KB), sen2)):
                KB.append(sen2)       
                known_map[x][y] = 0
            if(entails(conjoin(KB), sen3)):
                KB.append(sen3)
            if(entails(conjoin(KB), sen4)):
                KB.append(sen4)
